...
export function GlobalSearch() {
  ...
  return (
    <form className="relative hidden w-full max-w-md md:block" onSubmit={onSubmit}>
      <div className="flex h-10 items-center gap-2 rounded-md border bg-slate-50 px-3 shadow-inner">
      ...
      {focused ? (
        <div className="absolute left-0 right-0 top-12 z-50 overflow-hidden rounded-md border bg-white shadow-xl shadow-slate-200">
          {results.length ? (
            results.map((entry) => (
              <Link
                ...
                href={entry.href}
                className={cn(
                  "flex items-center justify-between gap-3 border-b px-3 py-2.5 text-sm transition-colors duration-150 last:border-b-0 hover:bg-blue-50/50",
